...
image_t fileToImage(FILE* image_file)
{
    unsigned int width_img = getWidth(image_file);
    unsigned int height_img = getHeight(image_file);
    image_t image = initImage(width_img,height_img);
    unsigned int x_bmp;
    unsigned int y_bmp;
    fseek(image_file,0x000A,SEEK_SET);

    unsigned int data_offset = getInt(image_file);
    printf("data_offset : %x\n",data_offset);
    fseek(image_file,data_offset,SEEK_SET);
    for(y_bmp=0; y_bmp<height_img; ++y_bmp)
    {
        for(x_bmp=0; x_bmp<width_img; ++x_bmp)
        {
            for(uchar col=0; col<3; ++col)
            {
                /* printf("(%d, %d)\n",x_bmp,y_bmp); */
                image[x_bmp][y_bmp][col] = fgetc(image_file);
            }
        }
    }
    return image;
...
char* chapronExtension(char input[])
{
    //Returns the name for BMP file with the ".chapron" extension
    char* output = NULL;
    int length = strlen(input);
    length +=7;
    output = malloc(length);
    strcpy(output,input);
    strcat(output,".chapron");
    return output;
}
